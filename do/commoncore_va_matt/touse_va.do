...
foreach subject in ela math {
	mark touse_g11_`subject' ///
		if grade==11 & dataset=="CAASPP" & test=="SBAC" & inrange(year, `test_score_min_year', `test_score_max_year') ///
...
foreach outcome in enr enr_2year enr_4year {
	mark touse_g11_`outcome' ///
		if grade==11 & dataset=="CAASPP" & test=="SBAC" & inrange(year, `outcome_min_year', `outcome_max_year') ///
		& diff_school_prop>=0.95
