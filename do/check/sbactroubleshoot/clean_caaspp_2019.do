*** do file cleancaaspp2019.do for Stata
*** version 16.0
*** by Kramer

/* 

This .do file cleans the 2018-2019 CAASP file (which contains 2018-2019 CADOE
...
This creates individual files for the following 2018-2019 assessments:
...
log using "/home/research/ca_ed_lab/msnaven/data/log_files/caaspp/clean_caaspp_2019.smcl", replace
set more off
version 16.0
...
******** 2018-2019
use "raw/caaspp/caasp1819.dta", clear
...
09  –  California Spanish Assessment  –  CSA
*/
tab rectype, missing
...
**** EL Status
* Sourced from CALPADS: English Learner Status
* Acceptable Values:
* Yes, No, or blank
/* 
...
tab el, mi
rename el x
label def ell 1 "English Learner" 0 "Not English Learner"
gen el:el = 1 if x=="Yes"
replace el = 0 if x=="No"
replace el = . if mi(x)
label var el "English Learner Status"
drop x
...
rename el_entrydate x
gen el_entry_date = date(x, "YMD")
replace el_entry_date = . if mi(x)
format el_entry_date %td
label var el_entry_date "Date student assigned as EL"
drop x
...
*** Primary Language
* Sourced from CALPADS: Language Code CALPADS 
* Acceptable Values:
* Alpha or Blank
tab primarylanguage, missing
destring primarylanguage, generate(primarylang) force
label val primarylang language
label var primarylang "Primary Language"
tab primarylang, missing
drop primarylanguage

*** Military Status
* Sourced from CALPADS: Military Status 
* Acceptable Values:
* Yes, No, or blank
tab military, missing
rename military x
generate military =.
replace military = 0 if x=="No"
replace military = 1 if x=="Yes"
label var military "Military Status"
tab military, missing
drop x

*** Foster Status
* Sourced from CALPADS: Foster Status
* Acceptable Values:
* Yes, No, or blank
tab foster, missing
rename foster x
generate foster = .
replace foster = 0 if x=="No"
replace foster = 1 if x=="Yes"
label var foster "Foster Status"
tab foster, missing
drop x

*** Homeless Status
* Sourced from CALPADS: Homeless Status
* Acceptable Values:
* Yes, No, or blank
tab homeless, missing
rename homeless x
generate homeless = .
replace homeless = 0 if x=="No"
replace homeless = 1 if x=="Yes"
label var homeless "Homeless Status"
tab homeless, missing
drop x
...
rename genre x
generate genre = .
replace genre=1 if x=="ARGU"
replace genre=2 if x=="EXPL"
replace genre=3 if x=="INFO"
replace genre=4 if x=="NARR"
replace genre=5 if x=="OPIN"
label def genrel 1 "Argumentative" ///
		 2 "Explanatory" ///
		 3 "Informational" ///
		 4 "Narrative" ///
		 5 "Opinion"

label val genre genrel
...
* Sourced from: Scoring 
* SB ELA                                           
* POR = Organization and Purpose
* Acceptable Values:
* 0, 1, 2, 3, 4, blank
tab wer_por, missing
rename wer_por x
destring x, generate(werpor)
tab werpor, missing
drop x
...
* Sourced from: Scoring 
* SB ELA                                           
* DEV EEL = Development/Evidence and Elaboration dimension
* Acceptable Values:
* 0, 1, 2, 3, 4, blank
tab wer_dev_eel, missing
rename wer_dev_eel x
destring x, generate(werdeveel)
tab werdeveel, missing
drop x

*** WER COV 
* Sourced from: Scoring 
* SB ELA                                           
* COV = Conventions dimension
* Acceptable Values:
* 0, 1, 2, 3, 4, blank
tab wer_cov, missing
rename wer_cov x
destring x, generate(wercov)
tab wercov, missing
drop x
...
compress
save "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caaspp_2019.dta", replace
...
******** SBAC ELA
use "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caaspp_2019.dta", clear
...
drop domain_2_level
drop domain_3_level
rename werpor sbac_ela_wer_por
rename werdeveel sbac_ela_wer_dev_eel
rename wercov sbac_ela_wer_cov
rename eap_college_release sbac_ela_eap_college_release
rename el limited_eng_prof
rename el_entry_date lep_entry_date
...
rename genre sbac_ela_genre
rename accommodations sbac_ela_accommodations

compress
save "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/sbac/sbac_2019_ela.dta", replace
...
compress
save "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/sbac/sbac_2019_math.dta", replace


******** Merge ELA/Math
use "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/sbac/sbac_2019_ela.dta", clear
merge 1:1 state_student_id using "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/sbac/sbac_2019_math.dta", nogen update

order state_student_id cdscode grade year first_name middle_name last_name birth_date
sort state_student_id grade year
compress
label data "Smarter Balanced Assessment Consortium, Academic Year 2018-2019"
save "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/sbac/sbac_2019.dta", replace
...
compress
save "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caa/caa_2019_ela.dta", replace
...
compress
save "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caa/caa_2019_math.dta", replace
...
compress
save "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caa/caa_2019_sci.dta", replace


******** Merge ELA/Math/Science
use "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caa/caa_2019_ela.dta", clear
merge 1:1 state_student_id using "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caa/caa_2019_math.dta", nogen update
merge 1:1 state_student_id using "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caa/caa_2019_sci.dta", nogen update

order state_student_id cdscode grade year first_name middle_name last_name birth_date
sort state_student_id grade year
compress
label data "California Alternate Assessments, Academic Year 2019-2019"
save "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caa/caa_2019.dta", replace
...
* Sourced from: Scoring
* CAST and CSA
* CAST Life Sciences Domain
* CSA Listening Domain
* Numeric or blank
* For CAST:
* 1 = Below Standard
* 2 =  Near Standard
...
* 1 = Not On Track
* 2 = On Track 
tab domain_1_level, missing
rename domain_1_level x
destring x, generate(domain_1_level)
label def domainl ///
	3 "Above Standard" ///
	2 "Near Standard" ///
	1 "Below Standard"
label val domain_1_level domainl
label var domain_1_level "Domain 1 Level"
tab domain_1_level, missing
drop x
...
* Sourced from: Scoring
* CAST and CSA
* CAST Physical Sciences Domain
* CSA Writing Mechanics Domain
* Numeric or blank
* For CAST:
* 1 = Below Standard
* 2 =  Near Standard
...
* 1 = Not On Track
* 2 = On Track 
tab domain_2_level, missing
rename domain_2_level x
destring x, generate(domain_2_level)
label val domain_2_level domainl
label var domain_2_level "Domain 2 Level"
tab domain_2_level, missing
drop x
...
* Sourced from: Scoring
* CAST and CSA
* CAST Earth and Space Sciences Domain
* CSA Reading Domain
* Numeric or blank
* For CAST:
* 1 = Below Standard
...
* 1 = Not On Track
* 2 = On Track 
tab domain_3_level, missing
rename domain_3_level x
destring x, generate(domain_3_level)
label val domain_3_level domainl
label var domain_3_level "Domain 3 Level"
tab domain_3_level, missing
drop x
...
drop sbac_error_band_max
drop achievementlevel
rename domain_1_level cast_domain_1_level
rename domain_2_level cast_domain_2_level
rename domain_3_level cast_domain_3_level
drop werpor
drop werdeveel
drop wercov
drop eap_college_release
...
sort state_student_id grade year
compress
label data "California Science Test, Academic Year 2018-2019"
save "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/cast/cast_2019.dta", replace




*** Put together the California Spanish Assessment—CSA file ********
******** CSA

use "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caaspp_2019.dta", clear
keep if caaspp_record_type==9
drop caaspp_record_type
...
rename grade_tested csa_grade_tested
rename oppidnpt csa_cat_opp_id
drop oppidpt
drop oppidsci
rename test_registration_id csa_test_registration_id
rename leanpt csa_cat_district
rename cdcodenpt csa_cat_cd_code
rename schnamenpt csa_cat_school
rename schcodenpt csa_cat_school_code
rename chartcodenpt csa_cat_charter_code
drop leapt
drop cdcodept
drop schnamept
drop schcodept
...
drop castind
drop theta
rename raw_score csa_raw_score
drop sbac_claim1_score
drop sbac_claim1_perf_level
drop sbac_claim2_score
drop sbac_claim2_perf_level
...
drop sbac_claim4_score
drop sbac_claim4_perf_level
rename scale_score csa_scale_score
drop sbac_std_error
drop sbac_error_band_min
drop sbac_error_band_max
rename achievementlevel csa_achievement_level
rename domain_1_level csa_domain_1_level
rename domain_2_level csa_domain_2_level
rename domain_3_level csa_domain_3_level
drop werpor
drop werdeveel
drop wercov
drop eap_college_release
...
rename el_entry_date lep_entry_date
drop field_test
rename oppstatnpt csa_cat_opp_status
rename chartnpt csa_cat_charter
rename npsnpt csa_cat_nonpub_nonsect
rename teststartnpt csa_cat_test_start_date
rename testendnpt csa_cat_test_end_date
drop oppstatpt
drop chartpt
drop npspt
drop teststartpt
...
drop testendsci
drop completion_date_paper
rename windowstartpt1 csa_cat_test_start_window
rename windowendpt1 csa_cat_test_end_window
drop windowstartpt2
drop windowendpt2
drop windowstartsci
drop windowendsci
drop castlastsci
rename condition_code csa_condition_code
rename test_attemptedness csa_test_attemptedness
rename score_status csa_score_status
rename urcc csa_urcc
rename test_mode csa_test_mode
rename include_indicator csa_include_indicator
drop genre
rename accommodations csa_accommodations

order state_student_id cdscode grade year first_name middle_name last_name birth_date
sort state_student_id grade year
compress
label data "California Spanish Assessment, Academic Year 2018-2019"
save "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/csa/csa_2019.dta", replace
...
erase "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caaspp_2019.dta"
erase "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/sbac/sbac_2019_ela.dta"
erase "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/sbac/sbac_2019_math.dta"
erase "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caa/caa_2019_ela.dta"
erase "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caa/caa_2019_math.dta"
erase "/home/research/ca_ed_lab/msnaven/data/restricted_access/clean/caaspp/caa/caa_2019_sci.dta"
